...
state_using_vector(int);        // 申明使用 vector_int_      =.=不能重复申明
// state_using_vector(double);     // 申明使用 vector_double_   =.=不能重复申明
// state_using_vector(char);       // 申明使用 vector_char_     =.=不能重复申明

// 要存放指针类型的需将 相应的指针类型typedef为另一个名字
// 同样!要结构体的需将 相应的结构体类型typedef为另一个名字
typedef char* pchar;
state_using_vector(pchar);          // 申明适用 vector_pchar_    =.=不能重复申明
